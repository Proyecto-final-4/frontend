...
export async function getBudgets(token: string): Promise<Budget[]> {
  ...
  if (!res.ok) await throwJavaApiErrorFromResponse(res, "Error al cargar presupuestos");
...
export async function getBudgetStatus(token: string, id: string): Promise<BudgetStatus> {
  ...
  if (!res.ok)
    await throwJavaApiErrorFromResponse(res, "Error al cargar el estado del presupuesto");
...
export async function createBudget(token: string, payload: CreateBudgetPayload): Promise<Budget> {
  ...
  if (!res.ok) await throwJavaApiErrorFromResponse(res, "Error al crear el presupuesto");
  return res.json() as Promise<Budget>;
}
...
export async function updateBudget(
  ...
  if (!res.ok) await throwJavaApiErrorFromResponse(res, "Error al actualizar el presupuesto");
  return res.json() as Promise<Budget>;
}
...
export async function deleteBudget(token: string, id: string): Promise<void> {
  ...
  if (!res.ok) await throwJavaApiErrorFromResponse(res, "Error al eliminar el presupuesto");
}
