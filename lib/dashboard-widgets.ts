...
import { getSummary, getSummaryTrends } from "@/sdk/summary";
import type { CategorySummary, SummaryResponse, SummaryTrendsResponse } from "@/types/summary";
...
export async function loadDashboardWidgetsData(
  token: string | null,
  ...
  try {
    const [summary, trends] = await Promise.all([
      getSummary(token, current.from, current.to),
      getSummaryTrends(token, {
    ...
    if (isEmptySummaryPeriod(summary)) return { kind: "empty" };
    return {
      kind: "data",
      summary,
      trends,
      topExpenses: topExpenseCategories(summary.expenseByCategory),
    };
  } catch {
    return { kind: "empty" };
  }
